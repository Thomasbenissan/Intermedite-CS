...
public class ErrorChecking {
    ...
    public static int errorCheck(Scanner sc) {
        ...
        System.out.println("I am expecting a number from 0 - 100!");

        if (sc.hasNextInt()) {
            int input = sc.nextInt();
            if (input > 100 || input < 0) { // if the number is not in between 1 and 100 (inclusive) print error message
                                            // that this was not the correct number
                System.out.println("Hey! this number is not what I wanted. Try again.");
                return errorCheck(sc); // it did not pass the error check, reiterate through the method (with the next
                                       // input)
            } else { // if the number is in between 1 and 100 (inclusive) print happy message
