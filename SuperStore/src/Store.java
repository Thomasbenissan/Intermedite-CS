        ...
        for (Item item : stocks) {
            if (item.name.equalsIgnoreCase(name)) {
                ...
                if(amount == 0){ //if they did not enter an int that passes error checkign
                    buyStock(stocks, userBalance, userItems);
                }
...
public static int getPositiveIntInput() {
    ...
    public static int sellStock(Item[] stocks, int userBalance, ArrayList<Item> userItems){
        ...
        userItems.forEach(item -> System.out.println(item.name + " " + item.amountOwned + " $" + item.amountOwned * item.price)); //cleaner for loop
        ...
        for (Item item : stocks) {
            if (item.name.equalsIgnoreCase(name)) {
                ...
                int amount = 0;
                if (sc.hasNextInt()) { //take the int that User puts in their next line even if surrounded by other chars
                    amount = sc.nextInt();
                } else {
    ...
    public static boolean keepPlaying(int userBalance){
        ...
        }else if(choice.equalsIgnoreCase("n")){ //stop playing
