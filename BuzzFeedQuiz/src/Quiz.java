...
public class Quiz {
    public static void main(String[] args) throws Exception {
        ...
        //finalAnswer depends on total quantity of points attributed to each answer chosen

        String finalAnswer = getAnswer(finalPoints); //set final answer to the sanwer corresponding ot the number of points which is figured out in the getAnswer method
        System.out.println("According to your personality, if you were a soccer player, you would be " + finalAnswer + "!");
    ...
    public static void gameIntro(Scanner sc) {
        ...
        String play = sc.next();
        if (!play.equals("1")){ // if the next line that the scanner takes as input, even if it is a string, is not '1' re iterate through the gameIntro method
    ...
    public static String getAnswer(int points){
       ...
       else if(points > 4 && points < 9){ //if the points from answers chosen add up to a number in betwen 4 and 9 (exclusive)
        finalAnswer = "Lionel Messi";
       }
       else{ //if the points from asnwers chosen add up to a number greater than 8 (exclusive)
